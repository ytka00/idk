...
def charfind(egyes,kettes,jel):
    ...
    for a in range(len(egyes):
        ...
        while kettes[a]!=charstr[z]:
            z+=1
        if jel=='-': 
            megoldas+=charstr[(z-j)%27]
        else:
            megoldas+=charstr[(j+z)%27]
    return(megoldas)
def rejtjelezes():
    uzenet = str(input('Adja meg az üzenetet! '))
    key = str(input('Adja meg a kulcsot! '))
    cipher = ''
    if all((ch in allowed for ch) in uzenet or key)==False:
        print('Az üzenet vagy kulcs hibás karaktereket tartalmaz')
        exit(1)
    if len(uzenet)>len(key):
        print('Hibás adatokat adott meg')
        exit(1)
    
    cipher=charfind(uzenet,key,'')
    print(cipher)
def megfejtes():
